...
app = Flask(__name__)
main_server = 'http://192.168.43.29:5000'
miner_address = None
...
class Miner:
    ...
    def sync_chain(self):
        ...
        if res.status_code == 200:
            ...
            self.pending = requests.get(f"{main_server}/tx/pending").json()
...
def receive_block():
    ...
    miner.pending = [tx for tx in miner.pending if tx not in block['transactions'][:-1]]
